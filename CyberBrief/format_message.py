...
def format_webex(articles: list[dict], context_line: str) -> str:
    ...
    for i, (article, emoji) in enumerate(zip(articles, _assign_unique_emojis(articles)), start=1):
        ...
        title_line = f"**{emoji} {i}. [{article['title']}]({link_url})** *· {source_part}*"
        lines += [title_line, "", article['tldr']]
    ...
    if _SHOW_LLM_NOTICE:
        lines += ["", "🤖 *Update: Summaries are now generated by Gemini 3.1 Flash Lite via the Cisco AI platform.*"]
...
def format_webex_card(articles: list[dict], context_line: str) -> dict:
    ...
    for i, (article, emoji) in enumerate(zip(articles, emojis), start=1):
        ...
        article_block = {
            ...
            "items": [
                {
                    "type": "TextBlock",
                    "text": f"{emoji} {i}. [{article['title']}]({link_url})",
                    "weight": "Default",
                    "color": "Accent",
                    "wrap": True,
                    "size": "Medium",
    ...
    if _SHOW_UPDATE_NOTICE:
        body.append(
            {
                "type": "TextBlock",
                "text": "🔔 Update: Article titles are now clickable links — tap the title to read the full story.",
                "isSubtle": True,
                "wrap": True,
                "spacing": "None",
                "size": "Small",
...
def format_telegram(articles: list[dict], context_line: str) -> str:
    ...
    for i, (article, emoji) in enumerate(zip(articles, _assign_unique_emojis(articles)), start=1):
        ...
        title_line = f"{emoji} <b>{i}. <a href=\"{link_url}\">{_escape_html(article['title'])}</a></b> <i>· {source_part}</i>"
        lines += [title_line, "", _escape_html(article['tldr'])]
        talk_track = article.get("talk_track")
        if talk_track:
    ...
    if _SHOW_LLM_NOTICE:
        lines += ["", "🤖 <i>Update: Summaries are now generated by Gemini 3.1 Flash Lite via the Cisco AI platform.</i>"]
